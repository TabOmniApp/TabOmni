...
type NoteState = {
   ...
   * than hiding what is filed into it. Here rather than in the sidebar because
   * `select` has to be able to open one: a note reaches the pane from the tab
   * strip and the search palette as often as from the list, and a row inside a
   * shut folder is a row nobody can see is marked.
   */
  collapsed: string[]
...
export const useNotes = create<NoteState>((set, get) => {
  ...
  return {
    ...
    select(id) {
      ...
      const { openIds, notes, folders } = get()

      // Everything above it, opened — see `collapsed`. Only ever opens: a
      // folder somebody shut stays shut unless the note is inside it.
      const chain = ancestorFolderIds(
        notes.find((note) => note.id === id)?.folderId ?? null,
        folders
      )
