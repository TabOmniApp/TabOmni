...
import { ancestorFolderIds } from "../tree"
...
type ApiState = {
   ...
   * Folders drawn shut.
   *
   * Collapsed rather than expanded ids, so a folder made later opens rather
   * than hiding what is put in it — the same bargain the Terminal store makes
   * with its own folds. It lives here rather than in the sidebar because
   * `select` has to be able to open a folder: what lands in the pane is
   * regularly chosen somewhere else entirely, and a list marking a row inside a
   * shut folder has marked nothing.
   */
  collapsed: string[]
 ...
 * resolution the MCP server needed, and `METHODS` followed it once the MCP
 * server could save a request as well as send one; re-exported so the panel
 * still reaches both where it always has.
 */
export { METHODS, variablesFrom } from "@shared/http-request"
...
export const useApi = create<ApiState>((set, get) => {
  ...
  return {
    ...
    toggleFolder(id) {
      set((state) => ({
        collapsed: state.collapsed.includes(id)
          ? state.collapsed.filter((entry) => entry !== id)
          : [...state.collapsed, id],
      }))
    },
    ...
    select(id) {
      ...
      const { openIds, requests, folders } = get()

      // Everything above it, opened — see `collapsed`. A request reaches the
      // pane from the tab strip and the palette as often as from the sidebar,
      // and a row inside a shut folder is a row nobody can see is marked.
      const chain = ancestorFolderIds(
        requests.find((request) => request.id === id)?.folderId ??
          // A folder can be the selected thing itself: its settings tab.
          (folders.some((folder) => folder.id === id) ? id : null),
        folders
      )
      ...
      set((state) => ({
        ...
        collapsed: state.collapsed.filter((entry) => !chain.includes(entry)),
      }))
      rememberTabs()
    },
