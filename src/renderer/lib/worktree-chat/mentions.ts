...
import { mentions, primeMentions } from "../mentions"
import type { PlainMention } from "./mention-text"

export { primeMentions }

/**
 * What a chat's `@` menu offers — the workspace's catalogue kept to the part a
 * name can carry, plus the databases themselves.
 *
 * Deliberately `lib/mentions.ts` rather than a second reader of the
 * same stores: what the workspace is holding is one question, and two answers to
 * it would drift the first time a panel changed. A row is `mydatabase.mytable`,
 * the name and nothing else, because the name is the whole reference. What this
 * adds on top of the catalogue is the one thing only a name can offer: the
 * databases themselves, connected to or not.
 */
export function chatMentions(): PlainMention[] {
