...
export function TerminalSidebar({ onAddFolder }: { onAddFolder: () => void }) {
   ...
   * ✕ uses it: closing the last session leaves this panel with nothing to show,
   * and what the pane goes to next is a question about the whole strip — a table
   * or a note may well still be open in it.
   *
   * "Close others" and "Close all" below stay the session list's own: this
   * sidebar is that list, and they mean the sessions rather than the workbench's
