 ...
 * key — the store holds those — the surrounding state was.
 */
...
/** What one tab of the panel shows: the settings page, a request, or a
 * folder's own defaults. */
function ApiPane({ id }: { id: string }) {
  const request = useApi((state) =>
    state.requests.find((candidate) => candidate.id === id)
  )
  const isFolder = useApi((state) =>
    state.folders.some((candidate) => candidate.id === id)
  )
  const outcome = useApi((state) => outcomeOf(state.outcomes, id))

  if (id === SETTINGS_TAB_ID) return <ApiSettings />
  ...
  // A tab whose request or folder has been deleted out from under it draws
  // nothing; the store drops the tab in the same tick.
  if (!isFolder) return null
  return <FolderSettingsEditor folderId={id} />
}
