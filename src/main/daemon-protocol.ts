 ...
 * agree on on Windows, so it is not "under `~/.tabula`" there — just a
 * name both processes know. */
export function socketPath(): string {
  if (process.platform === "win32") return "\\\\.\\pipe\\tabula-agent-daemon"
  return path.join(homedir(), ".tabula", "agent-daemon.sock")
}
