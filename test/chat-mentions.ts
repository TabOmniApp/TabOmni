 ...
 *
 * Nothing is rewritten on the way out — the name typed is the name sent — so
 * what is worth testing is the two things that are silent when they are wrong:
 * which text opens a menu (an email address must not), and which runs are
 * tinted. A tint on the wrong run claims the workspace holds something it does
 * not.
 */
