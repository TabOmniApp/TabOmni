...
section("drawingIdsIn")
{
  ...
  const note = [text("Architecture"), drawing(ID), text("prose"), drawing(OTHER)]
  check(
  ...
  check("a drawing nested under another block", drawingIdsIn(nested).join() === ID)

  check("a note with no drawing has none", drawingIdsIn([text("words")]).length === 0)
